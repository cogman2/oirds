...
def main():
    ...
    for i,r in xlsInfos.iterrows():
       if (lastname!= r[2] and len(lastList) > 0):
          if(r[6]==1):
            initialSize, rawImage = loadImg(lastname, xlsDir)
            result = runcaffe(parentDataDir,rawImage)
...
def loadImg(name,dir):
  print name + '-----------'
  initialSize,im = gt_tool.loadImage(dir + 'png/' + name[0:name.index('.tif')] + '.png')
...
def runcaffe (dataDir,im):
   ...
   dumpNetWeights(net)
   return outputResult(net.forward(), transformer, net.blobs['data'].data[0])
